...
- (void)setupUI {
    [_replayButton setTitle:@"开始" forState:UIControlStateNormal];
...
- (void)gameOverOperaton {
    NSLog(@"---- Game Over ----");
    [SVProgressHUD showErrorWithStatus:@"Game Over"];
    [self performSelector:@selector(dismissSVPHUD) withObject:nil afterDelay:1.0];
...
- (void)commitRefreshAnimation {
    ...
    self.refreshTimer = [NSTimer scheduledTimerWithTimeInterval:0.04 target:self selector:@selector(refresh) userInfo:nil repeats:YES];
...
- (IBAction)rotate:(id)sender {
    ...
    [_replayButton setTitle:@"重玩" forState:UIControlStateNormal];
    if ([self isPauseState]) return;
...
- (IBAction)pause:(UIButton *)sender {
    [sender setTitle:sender.selected ? (@"暂停" ): (@"开始" ) forState:UIControlStateNormal];
