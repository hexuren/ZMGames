...
- (void)initData {
    ...
    self.height = self.view.frame.size.height - 64;
...
- (void)initUI {
    ...
    _mainView = [[UIView alloc] initWithFrame:CGRectMake(margin_x, 64, mainViewWidth , mainViewWidth)];
    _mainView.backgroundColor = [UIColor blackColor];
    [self.view addSubview:_mainView];
    
    _zj = [[UILabel alloc] initWithFrame:CGRectMake(-20, -10-30, 100, 40)];
    _zj.text = @"主角光环";
    ...
    _info = [[UILabel alloc] initWithFrame:CGRectMake(mainViewWidth / 2, self.width - 20 + marginTop - 5 + 30, mainViewWidth / 2, btnWidth)];
    _info.textAlignment = NSTextAlignmentLeft;
    _info.textColor = [UIColor orangeColor];
    
    [_info setFont:[UIFont fontWithName:@"Helvetica-BoldOblique" size:30]];
    ...
    UIImageView *iv = [[UIImageView alloc] initWithFrame:CGRectMake(mainViewWidth / 2 - btnWidth, self.width - 20 + marginTop + 30, 30, 30)];
...
- (void)refresh:(NSTimer *)timer {
    ...
    if (_mainTank.life > 0 && _mainTank.isDeath) {
        if (_mainTank.life > 0) {
            ...
            _mainTank.life = life;
            _info.text = [NSString stringWithFormat:@"我方：%d", _mainTank.life];
            [self zjgh];
        }
    }
    if (_mainTank.life <= 0 && _mainTank.isDeath) {
